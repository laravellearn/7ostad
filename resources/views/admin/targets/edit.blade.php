            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-4 box-margin">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title mb-2">ویرایش برنامه هدف</h4><hr>
                                <form action="{{ route('targets.update',['target'=>$target->id]) }}" method="POST">
                                    @CSRF
                                    @method('PATCH')
                                    <div class="form-group">
                                        <label class="control-label">نام برنامه*</label>
                                        <input type="text" name="title" value="{{ $target->title }}" required class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label>تاریخ شروع*</label>
                                        <input type="text" id="input1" value="{{ $target->start_date }}" name="start_date" required  class="form-control">
                                        <span id="span1"></span>
                                    </div>
                                    <div class="form-group">
                                        <label>تاریخ پایان*</label>
                                        <input type="text" id="input2" value="{{ $target->end_date }}" name="end_date" required  class="form-control">
                                        <span id="span2"></span>
                                    </div>
                                    ...
                                    @foreach($targets as $target)
                                        <tr>
                                            ...
                                            <td style="text-align: center;padding-top: 2px" class="d-flex">
                                                <a href="{{ route('targets.edit',['target'=>$target->id]) }}" style="margin-top:2px;margin-left:6px">
