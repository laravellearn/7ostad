...
@section('title','ویرایش درس در برنامه هدف')
@section('style')
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-4 box-margin">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title mb-2">ویرایش درس در برنامه هدف</h4><hr>
                                <form action="{{ route('subtargets.update',['subtarget'=>$subtarget->id]) }}" method="POST">
                                    @CSRF
                                    @method('PATCH')
                                    <div class="form-group">
                                        ...
                                        <select name="target_id" class="form-control form-control-sm mb-3 my_select1" required>
                                            @foreach($targets as $target)
                                                <option value="{{ $target->id }}" {{ $target->id==$subtarget->target_id ? 'selected' : '' }}>{{ $target->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select name="book_id" class="form-control form-control-sm mb-3 my_select2" required>
                                            @foreach($books as $book)
                                                <option value="{{ $book->id }}" {{ $book->id==$subtarget->book_id ? 'selected' : '' }}>{{ $book->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select name="topic_id" class="form-control form-control-sm mb-3 my_select3" required>
                                            @foreach($topics as $topic)
                                                <option value="{{ $topic->id }}" {{ $topic->id==$subtarget->topic_id ? 'selected' : '' }}>{{ $topic->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
